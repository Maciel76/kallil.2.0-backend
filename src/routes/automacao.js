...
router.use((req, res, next) => {
  if (req.userRole === "operador") {
    return res
      .status(403)
      .json({ message: "Operadores não têm acesso à automação." });
  }
  next();
});
...
router.use(async (req, res, next) => {
  try {
    ...
    const user = await User.findById(req.userId).select(
      "planoWhatsapp whatsappAssinaturaExpira",
    );
    if (!user)
    ...
    const ativo =
      ...
      user.whatsappAssinaturaExpira > new Date();
    if (!ativo) {
  ...
  try {
    ...
    if (exists)
      return res
        .status(400)
        .json({
          message: "Você já possui uma instância. Remova antes de criar outra.",
        });
...
router.post("/instance/connect", async (req, res) => {
  try {
    ...
    whatsapp
      .initialize()
      .catch((err) => console.error("[AUTOMACAO] init err:", err.message));
...
router.get("/instance/qr-stream", async (req, res) => {
  try {
    ...
    if (!wa) {
      res.write(
        `data: ${JSON.stringify({ type: "error", message: "Sessão inexistente. Clique Conectar." })}\n\n`,
      );
      return res.end();
    }
    if (wa.qrCode)
      res.write(
        `data: ${JSON.stringify({ type: "qr", qrCode: wa.qrCode })}\n\n`,
      );
    if (wa.connected) {
      res.write(
    ...
    const onQr = (qr) =>
      res.write(`data: ${JSON.stringify({ type: "qr", qrCode: qr })}\n\n`);
...
router.get("/notificacoes", async (req, res) => {
  try {
    const instance = await WhatsAppInstance.findOne({ userId: req.userRealId });
    if (!instance)
      return res.json({ notificacoes: null, notificationPhone: "" });
    res.json({
...
router.put("/notificacoes", async (req, res) => {
  try {
    ...
    if (!instance)
      return res
        .status(404)
        .json({ message: "Configure uma instância primeiro." });
...
router.post("/test-message", async (req, res) => {
  try {
    ...
    if (!number || !message)
      return res
        .status(400)
        .json({ message: "number e message obrigatórios." });
    const instance = await WhatsAppInstance.findOne({ userId: req.userRealId });
    if (!instance)
      return res.status(404).json({ message: "Instância não encontrada." });
    if (instance.status !== "connected")
    ...
    const wa = activeSessions.get(instance._id.toString());
    if (!wa || !wa.connected)
      return res.status(400).json({ message: "Sessão inativa. Reconecte." });
...
router.post("/enviar-cupom", async (req, res) => {
  try {
    ...
    if (!vendaId || !number)
      return res
        .status(400)
        .json({ message: "vendaId e number são obrigatórios." });
...
router.post("/workflows", async (req, res) => {
  try {
    ...
    if (!name || !instanceId)
      return res.status(400).json({ error: "Nome e instância obrigatórios." });
    const inst = await WhatsAppInstance.findOne({
      _id: instanceId,
      userId: req.userRealId,
    });
      ...
      nodes: nodes || [
        {
          ...
          type: "trigger",
          position: { x: 250, y: 50 },
          data: { triggerType: "all", triggerValue: "" },
        },
      ],
      edges: edges || [],
    });
...
router.delete("/workflows/:id([a-fA-F0-9]{24})", async (req, res) => {
  try {
    const wf = await Workflow.findOneAndDelete({
      _id: req.params.id,
      userId: req.userRealId,
    });
    if (!wf) return res.status(404).json({ error: "Workflow não encontrado." });
    await WorkflowSession.updateMany(
      { workflowId: wf._id, status: "active" },
      { status: "completed" },
    );
  ...
  res.json([
    {
      id: "boas-vindas",
      nome: "👋 Boas-vindas",
      desc: "Saúda novos contatos pela primeira vez",
    ...
    },
    {
      ...
      desc: "Menu interativo de atendimento automático",
    ...
    },
    {
      id: "pos-venda",
      nome: "🛍️ Pós-Venda",
      desc: "Pesquisa de satisfação após compra",
      icone: "fas fa-star",
      cor: "#f59e0b",
      tag: "⭐ Avaliações",
      categoria: "Pós-Venda",
    ...
    {
      id: "cobranca",
      nome: "💰 Cobrança Educada",
      desc: "Lembrete educado de pagamento em aberto",
    ...
    },
    {
      id: "recuperacao",
      nome: "🎯 Recuperação",
      desc: "Reativa clientes que estão sem comprar",
    ...
    },
    {
      ...
      desc: "Mensagem especial com cupom no aniversário",
    ...
    },
    {
      id: "pesquisa",
      nome: "⭐ Pesquisa NPS",
      desc: "Coleta feedback (nota + comentário)",
      icone: "fas fa-poll",
      cor: "#06b6d4",
      tag: "📊 Insights",
      categoria: "Pós-Venda",
    },
    {
      ...
      desc: "Resposta automática quando você está offline",
      icone: "fas fa-moon",
      cor: "#6366f1",
      tag: "🌙 24/7",
      categoria: "Atendimento",
    },
    {
      id: "agendamento",
      nome: "📅 Agendamento",
      desc: "Coleta dados para marcar horário/serviço",
    ...
    },
    {
      ...
      desc: "Mostra cardápio e recebe pedidos via menu",
    ...
    },
    {
      ...
      desc: "Coleta nome, telefone e interesse para vendedor",
      icone: "fas fa-magnet",
      cor: "#a855f7",
      tag: "💎 Premium",
      categoria: "Vendas",
    ...
    {
      id: "rastreio",
      nome: "📦 Rastreio de Pedido",
      desc: "Cliente consulta status do pedido pelo WhatsApp",
      icone: "fas fa-truck",
      cor: "#0891b2",
      tag: "🚚 Logística",
      categoria: "Pós-Venda",
    },
    {
      ...
      desc: "Avisa cliente quando todos atendentes estão ocupados",
    ...
    {
      id: "departamentos",
      nome: "🏢 Direcionar Setor",
      desc: "Cliente escolhe Vendas, Suporte ou Financeiro",
    ...
    {
      id: "suporte-tecnico",
      nome: "🛠️ Suporte Técnico",
      desc: "Triagem do problema antes de chamar atendente",
    ...
    {
      id: "confirma-presenca",
      nome: "✅ Confirmar Presença",
      desc: "Confirma agendamento de consulta/serviço",
      icone: "fas fa-calendar-check",
      cor: "#059669",
      tag: "📲 Reduz no-show",
      categoria: "Atendimento",
    },
    {
      ...
      desc: "Lembra quem não finalizou a compra",
    ...
    },
    {
      ...
      desc: "Lembrete 3 dias antes do vencimento",
    ...
    },
    {
      ...
      desc: "Programa de indicação com cupom para os dois",
    ...
    },
    {
      ...
      desc: "Volta clientes inativos há 30+ dias",
      icone: "fas fa-rotate",
      cor: "#0284c7",
      tag: "📈 Retenção",
      categoria: "Marketing",
    },
    {
      ...
      desc: "Dispara promoção com prazo limitado",
      icone: "fas fa-bolt",
      cor: "#fbbf24",
      tag: "🔥 Urgência",
      categoria: "Marketing",
    ...
    {
      id: "cancelamento",
      nome: "🛡️ Retenção/Cancelamento",
      desc: "Tenta reter cliente que quer cancelar",
...
router.post("/workflows/templates", async (req, res) => {
  try {
    ...
    if (!instanceId)
      return res.status(400).json({ error: "Instância obrigatória." });
    ...
    const templates = {
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "all" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! 👋 Bem-vindo!" },
          },
          {
            id: "menu-1",
            type: "menu",
            ...
            data: {
              ...
              options: [
                { id: "opt-1", label: "Informações" },
                { id: "opt-2", label: "Falar com atendente" },
              ],
            },
          },
          {
            ...
            type: "sendMessage",
            position: { x: 0, y: 440 },
            data: { message: "ℹ️ Horário: Seg–Sex 9h–18h." },
          },
          {
            id: "msg-humano",
            type: "sendMessage",
            position: { x: 500, y: 440 },
            data: { message: "👤 Um atendente já vai falar com você." },
          },
      ...
      "pos-venda": {
        ...
        nodes: [
          ...
          },
          {
            ...
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! Você comprou conosco recentemente." },
          },
          {
            id: "wait-1",
            type: "waitForReply",
            ...
            data: {
              ...
              message: "De 1 a 5, como você avaliaria sua experiência?",
            },
          },
          {
            id: "msg-2",
            ...
            data: { message: "Obrigado pela avaliação ({avaliacao})! 🙏" },
          },
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "firstContact" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá! 👋 Seja muito bem-vindo(a)!" },
          },
          {
            id: "delay-1",
            type: "delay",
            position: { x: 250, y: 240 },
            data: { unit: "seconds", value: 3 },
          },
          {
            id: "msg-2",
            type: "sendMessage",
            ...
            data: {
              message: "Sou seu atendente automático. Como posso ajudar hoje?",
            },
          },
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "pagamento" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! Tudo bem?" },
          },
          {
            id: "msg-2",
            type: "sendMessage",
            ...
            data: {
              message:
                "Notamos que você tem um pagamento de R$ {valor} com vencimento em {vencimento}. 💳",
            },
          },
          {
            id: "menu-1",
            ...
            data: {
              ...
              options: [
                { id: "opt-1", label: "Já paguei" },
                { id: "opt-2", label: "Quero negociar" },
              ],
            },
          },
          {
            ...
            type: "sendMessage",
            position: { x: 0, y: 580 },
            data: { message: "Ótimo! Vamos verificar e te avisar. Obrigado!" },
          },
          {
            id: "msg-neg",
            type: "sendMessage",
            ...
            data: {
              message: "Sem problemas! Um atendente vai entrar em contato.",
            },
          },
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "voltar" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Oi {nome}! Sentimos sua falta por aqui 💚" },
          },
          {
            id: "msg-2",
            type: "sendMessage",
            ...
            data: {
              message:
                "Para te receber de volta, preparamos um cupom exclusivo: *VOLTOU10* — 10% OFF na sua próxima compra!",
            },
          },
          {
            id: "tag-1",
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "aniversario" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "🎉 PARABÉNS, {nome}! 🎂" },
          },
          {
            id: "msg-2",
            type: "sendMessage",
            ...
            data: {
              message:
                "Para comemorar com você, presenteamos com um cupom de 15% OFF: *NIVER15* (válido por 7 dias).",
            },
          },
          {
            id: "tag-1",
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "feedback" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: {
              message: "Olá {nome}! Sua opinião é muito importante para nós.",
            },
          },
          {
            id: "wait-1",
            ...
            data: {
              variableName: "nota",
              message: "De 1 a 10, qual a chance de você nos recomendar?",
            },
          },
          {
            id: "wait-2",
            ...
            data: {
              ...
              message: "Quer deixar algum comentário?",
            },
          },
          {
            id: "msg-2",
            ...
            data: {
              message: "Obrigado pelo feedback! Sua nota foi {nota}. 💚",
            },
          },
      ...
      horario: {
        ...
        nodes: [
          ...
          },
          {
            ...
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! 🌙 Recebemos sua mensagem." },
          },
          {
            id: "msg-2",
            type: "sendMessage",
            ...
            data: {
              message:
                "Estamos fora do horário de atendimento. Nosso horário: Seg–Sex 9h–18h, Sáb 9h–13h.",
            },
          },
          {
            id: "msg-3",
            ...
            position: { x: 250, y: 400 },
            data: {
              message: "Retornaremos seu contato no próximo expediente. 💚",
            },
          },
          { id: "end-1", type: "end", position: { x: 250, y: 520 }, data: {} },
        ],
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "agendar" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! Vamos agendar seu horário. 📅" },
          },
          {
            id: "wait-1",
            type: "waitForReply",
          ...
          },
          {
            ...
            position: { x: 250, y: 400 },
            data: {
              variableName: "data_pref",
              message: "Qual data e horário você prefere? (ex: 12/05 às 14h)",
            },
          },
          {
            id: "msg-2",
          ...
          },
          {
            ...
            data: { tag: "aguardando-agendamento" },
          },
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "cardapio" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! 🍕 Confira nosso cardápio:" },
          },
          {
            id: "menu-1",
            type: "menu",
            position: { x: 250, y: 260 },
            data: {
              message: "Qual categoria deseja?",
              options: [
                { id: "opt-1", label: "Pizzas 🍕" },
                { id: "opt-2", label: "Lanches 🍔" },
                { id: "opt-3", label: "Bebidas 🥤" },
              ],
            },
          },
          {
          ...
          },
          {
            ...
            data: { variableName: "pedido", message: "O que vai querer?" },
          },
          {
            id: "msg-confirma",
            type: "sendMessage",
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "orcamento" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            ...
            data: {
              message:
                "Olá! Que ótimo seu interesse. 🎯 Para passar para um especialista, preciso de algumas informações.",
            },
          },
          {
            id: "wait-1",
          ...
          },
          {
            ...
            data: {
              ...
              message: "O que você procura? (descreva brevemente)",
            },
          },
          {
            id: "wait-3",
            ...
            data: {
              ...
              message:
                "Quando pretende contratar? (esta semana, mês, sem pressa)",
            },
          },
          {
            id: "msg-2",
            ...
            data: {
              message:
                "Perfeito {nome_lead}! 🎉 Recebi seus dados e um especialista entrará em contato em até 1 hora útil.",
            },
          },
          {
            id: "tag-1",
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "rastrear" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! 📦 Vamos rastrear seu pedido." },
          },
          {
            id: "wait-1",
            type: "waitForReply",
            ...
            data: {
              ...
              message: "Qual o número do seu pedido?",
            },
          },
          {
            id: "msg-2",
          ...
          },
          {
            ...
            data: {
              message: "Escolha:",
              options: [
                { id: "opt-1", label: "Sim, falar com atendente" },
                { id: "opt-2", label: "Não, obrigado" },
              ],
            },
          },
          {
          ...
          },
          {
            ...
            data: { message: "Tudo bem! Qualquer coisa é só chamar. 💚" },
          },
      ...
      "fila-atendimento": {
        ...
        description:
          "Avisa cliente que está em fila quando todos atendentes estão ocupados",
        nodes: [
          {
            id: "trigger-1",
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "atendente" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: {
              message: "Olá {nome}! 👋 Você está na fila de atendimento.",
            },
          },
          {
            id: "msg-2",
          ...
          },
          {
            ...
            type: "addTag",
            position: { x: 250, y: 400 },
            data: { tag: "fila-espera" },
          },
          {
            id: "delay-1",
            type: "delay",
            position: { x: 250, y: 520 },
            data: { duration: 300, unit: "seconds" },
          },
          {
            id: "msg-3",
            type: "sendMessage",
            ...
            data: {
              message:
                "Obrigado pela paciência {nome}! Um atendente estará com você em instantes. 🙏",
            },
          },
      ...
      departamentos: {
        ...
        nodes: [
          ...
          },
          {
            ...
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! 👋 Bem-vindo ao nosso atendimento." },
          },
          {
            id: "menu-1",
            type: "menu",
            ...
            data: {
              ...
              options: [
                { id: "opt-1", label: "🛒 Vendas / Comprar" },
                { id: "opt-2", label: "🛠️ Suporte / Problemas" },
                { id: "opt-3", label: "💳 Financeiro / Boletos" },
                { id: "opt-4", label: "👤 Outro assunto" },
              ],
            },
          },
          {
            ...
            data: {
              message:
                "🛒 Encaminhando você para o time de *Vendas*. Em instantes um consultor responderá!",
            },
          },
          {
            id: "tag-vendas",
          ...
          },
          {
            ...
            data: {
              message:
                "🛠️ Encaminhando para *Suporte Técnico*. Tenha em mãos detalhes do problema.",
            },
          },
          {
            id: "tag-suporte",
          ...
          },
          {
            ...
            data: {
              message:
                "💳 Encaminhando para *Financeiro*. Aguarde o retorno em até 1 hora útil.",
            },
          },
          {
            id: "tag-fin",
          ...
          },
          {
            ...
            data: {
              message: "👤 Um atendente irá falar com você em instantes.",
            },
          },
          {
            id: "tag-outro",
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "suporte" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: {
              message: "Olá {nome}! 🛠️ Vamos resolver seu problema rapidinho.",
            },
          },
          {
            id: "menu-1",
            ...
            position: { x: 250, y: 260 },
            data: {
              message: "Qual o tipo do problema?",
              options: [
                { id: "opt-1", label: "🔌 Não liga / sem energia" },
                { id: "opt-2", label: "📶 Sem conexão / internet" },
                { id: "opt-3", label: "⚙️ Funcionamento incorreto" },
                { id: "opt-4", label: "❓ Outro" },
              ],
            },
          },
          {
          ...
          {
            id: "msg-funcionamento",
            type: "sendMessage",
            position: { x: 500, y: 480 },
            data: {
              message:
                "⚙️ Para te ajudar melhor, descreva em detalhes o que está acontecendo.",
            },
          },
          {
            id: "msg-outro",
            ...
            data: {
              message:
                "❓ Sem problema! Descreva sua questão e um técnico vai te ajudar.",
            },
          },
          {
            id: "wait-desc",
      ...
      "confirma-presenca": {
        ...
        nodes: [
          ...
          },
          {
            ...
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Olá {nome}! 📅 Lembrete do seu agendamento:" },
          },
          {
            id: "msg-2",
            type: "sendMessage",
          ...
          },
          {
            ...
            data: {
              ...
              options: [
                { id: "opt-1", label: "✅ Sim, confirmo" },
                { id: "opt-2", label: "🔄 Preciso remarcar" },
                { id: "opt-3", label: "❌ Cancelar" },
              ],
            },
          },
          {
            ...
            data: {
              message:
                "✨ Perfeito! Te esperamos no horário marcado. Até lá! 💚",
            },
          },
          {
            id: "tag-conf",
          ...
          },
          {
            ...
            data: {
              message:
                "🔄 Sem problemas! Um atendente vai falar com você para encontrar uma nova data.",
            },
          },
          {
            id: "tag-rem",
          ...
          },
          {
            ...
            data: {
              message:
                "😢 Lamentamos! Seu agendamento foi cancelado. Quando quiser, é só chamar.",
            },
          },
          {
            id: "tag-canc",
            ...
            data: { tag: "agendamento-cancelado" },
          },
      ...
      "carrinho-abandonado": {
        ...
        nodes: [
          ...
          },
          {
            ...
            position: { x: 250, y: 120 },
            data: {
              message: "Oi {nome}! 👋 Você esqueceu algo no seu carrinho... 🛒",
            },
          },
          {
            id: "msg-2",
          ...
          },
          {
            ...
            type: "delay",
            position: { x: 250, y: 400 },
            data: { duration: 3600, unit: "seconds" },
          },
          {
            id: "menu-1",
            type: "menu",
            position: { x: 250, y: 520 },
            data: {
              message: "Conseguiu finalizar?",
              options: [
                { id: "opt-1", label: "✅ Sim, finalizei" },
                { id: "opt-2", label: "🤔 Tive dúvidas" },
                { id: "opt-3", label: "❌ Não vou comprar" },
              ],
            },
          },
          {
            ...
            data: { message: "Ótimo! 🎉 Em breve seu pedido chega. Obrigado!" },
          },
          {
            id: "msg-duvida",
            type: "sendMessage",
            ...
            data: {
              message:
                "Sem problema! Um atendente vai te ajudar com as dúvidas em instantes.",
            },
          },
          {
            id: "tag-duvida",
            ...
            data: { tag: "duvida-compra" },
          },
          {
            id: "msg-no",
            type: "sendMessage",
            position: { x: 600, y: 720 },
            data: { message: "Tudo bem! Quando quiser, estamos aqui. 💚" },
          },
          {
            id: "tag-no",
            type: "addTag",
            position: { x: 600, y: 860 },
            data: { tag: "carrinho-perdido" },
          },
          { id: "end-1", type: "end", position: { x: 250, y: 1000 }, data: {} },
        ],
        edges: [
      ...
      "boleto-vencendo": {
        ...
        nodes: [
          ...
          },
          {
            ...
            position: { x: 250, y: 120 },
            data: {
              message: "Olá {nome}! 👋 Lembrete amigável sobre seu boleto.",
            },
          },
          {
            id: "msg-2",
          ...
          },
          {
            ...
            data: {
              ...
              options: [
                { id: "opt-1", label: "✅ Já paguei" },
                { id: "opt-2", label: "📲 Reenviar boleto" },
                { id: "opt-3", label: "🔄 Negociar prazo" },
              ],
            },
          },
          {
            ...
            data: {
              message:
                "Ótimo! 🙌 Vamos confirmar no sistema. Obrigado pela parceria!",
            },
          },
          {
            id: "msg-reenv",
            ...
            data: {
              message:
                "📲 Em instantes você receberá o boleto novamente. Aguarde!",
            },
          },
          {
            id: "tag-reenv",
          ...
          },
          {
            ...
            data: {
              message:
                "🤝 Entendido! Um atendente do financeiro vai conversar com você.",
            },
          },
          {
            id: "tag-neg",
        ...
        nodes: [
          {
            ...
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "indicar" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Que demais {nome}! 🎉 Adoramos indicações." },
          },
          {
            id: "msg-2",
            type: "sendMessage",
          ...
          },
          {
            ...
            position: { x: 250, y: 400 },
            data: {
              variableName: "amigo_nome",
              message: "Qual o nome do seu amigo?",
            },
          },
          {
            id: "wait-2",
            ...
            position: { x: 250, y: 540 },
            data: {
              variableName: "amigo_telefone",
              message: "E o telefone dele? (com DDD)",
            },
          },
          {
            id: "msg-3",
      ...
      "reativacao-30": {
        ...
        nodes: [
          ...
          },
          {
            ...
            position: { x: 250, y: 120 },
            data: {
              message: "Oi {nome}! 👋 Faz tempo que não te vejo por aqui...",
            },
          },
          {
            id: "msg-2",
            ...
            data: {
              message:
                "🎁 Para te trazer de volta, separei um *cupom exclusivo*: \n\n🏷️ *VOLTA20* — 20% OFF (válido por 7 dias)",
            },
          },
          {
            id: "menu-1",
            ...
            position: { x: 250, y: 400 },
            data: {
              message: "Quer aproveitar?",
              options: [
                { id: "opt-1", label: "🛒 Sim, ver novidades!" },
                { id: "opt-2", label: "🤔 Quero mais detalhes" },
                { id: "opt-3", label: "🚫 Não, obrigado" },
              ],
            },
          },
          {
            ...
            data: {
              message:
                "🔥 Vou te mandar nossas novidades agora! Em instantes...",
            },
          },
          {
            id: "tag-ver",
            type: "addTag",
            position: { x: -100, y: 740 },
            data: { tag: "reativado-interessado" },
          },
          {
            id: "msg-det",
            type: "sendMessage",
            ...
            data: {
              message:
                "Um atendente vai conversar com você e tirar todas as dúvidas. 💚",
            },
          },
          {
            id: "tag-det",
          ...
          },
          {
            ...
            position: { x: 600, y: 600 },
            data: {
              message: "Tudo bem! Quando precisar de algo, é só chamar. 😊",
            },
          },
          {
            id: "tag-nao",
      ...
      "promocao-flash": {
        ...
        description:
          "Dispara promoção com prazo limitado e gatilho de urgência",
        nodes: [
          {
            id: "trigger-1",
            type: "trigger",
          ...
          },
          {
            ...
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "🚨 *PROMOÇÃO RELÂMPAGO* 🚨" },
          },
          {
            id: "msg-2",
            type: "sendMessage",
          ...
          },
          {
            ...
            position: { x: 250, y: 400 },
            data: {
              message: "Quer garantir o seu?",
              options: [
                { id: "opt-1", label: "🛒 Quero comprar AGORA" },
                { id: "opt-2", label: "📷 Ver fotos primeiro" },
                { id: "opt-3", label: "⏰ Avise antes de acabar" },
              ],
            },
          },
          {
            ...
            position: { x: -100, y: 600 },
            data: {
              message: "Boa! 🎉 Um vendedor já vai falar com você para fechar.",
            },
          },
          {
            id: "tag-comprar",
            type: "addTag",
            position: { x: -100, y: 740 },
            data: { tag: "promo-quente" },
          },
          {
            id: "msg-fotos",
            type: "sendMessage",
            position: { x: 250, y: 600 },
            data: { message: "📷 Em instantes te mando as fotos!" },
          },
          {
            id: "tag-fotos",
            type: "addTag",
            position: { x: 250, y: 740 },
            data: { tag: "promo-interessado" },
          },
          {
            id: "msg-aviso",
            type: "sendMessage",
            position: { x: 600, y: 600 },
            data: {
              message: "⏰ Combinado! Te aviso meia hora antes de acabar. 🙌",
            },
          },
          {
            id: "tag-aviso",
      ...
      cancelamento: {
        ...
        description:
          "Tenta reter cliente que quer cancelar oferecendo benefícios",
        nodes: [
          {
            id: "trigger-1",
            type: "trigger",
            position: { x: 250, y: 0 },
            data: { triggerType: "keyword", keyword: "cancelar" },
          },
          {
            id: "msg-1",
            type: "sendMessage",
            position: { x: 250, y: 120 },
            data: { message: "Poxa {nome}, sentimos muito! 😔" },
          },
          {
            id: "wait-1",
            type: "waitForReply",
            position: { x: 250, y: 260 },
            data: {
              variableName: "motivo_cancel",
              message:
                "Pode nos contar o que aconteceu? Sua opinião nos ajuda muito.",
            },
          },
          {
            id: "msg-2",
            ...
            data: {
              message:
                "Entendido. 🙏 Antes de prosseguir, temos uma proposta especial pra você:",
            },
          },
          {
            id: "msg-3",
          ...
          },
          {
            ...
            position: { x: 250, y: 680 },
            data: {
              message: "O que prefere?",
              options: [
                { id: "opt-1", label: "🎁 Aceito a oferta" },
                { id: "opt-2", label: "💬 Quero falar com gerente" },
                { id: "opt-3", label: "❌ Cancelar mesmo assim" },
              ],
            },
          },
          {
            ...
            data: {
              message:
                "🎉 Que ótimo! Já estamos aplicando o desconto. Bem-vindo de volta!",
            },
          },
          {
            id: "tag-ret",
          ...
          },
          {
            ...
            data: {
              message:
                "💼 Um gerente vai conversar com você em até 1 hora. Vamos resolver!",
            },
          },
          {
            id: "tag-ger",
          ...
          },
          {
            ...
            data: {
              message:
                "😢 Entendemos. Em instantes processamos seu cancelamento. Foi um prazer te atender!",
            },
          },
          {
            id: "tag-cancel",
